  ...
  if (data.items.length === 0) {
    return (
      <div className={styles.card}>
        <div className={styles.cardTitle}><Text variant="h4">Participación de top ventas</Text></div>
        <div className={styles.cardBody}><Text variant="description">No hay datos de ventas.</Text></div>
      </div>
    );
  }
                  ...
                  <tr key={i}>
                    <td>
                      <div className={styles.sellerNameCell}>
                        <span className={`${styles.sellerColorDot} ${styles.dotColor}`} style={{ '--dot-color': COLORS[i] } as React.CSSProperties} />
                        <Text variant="bodySm" weight="semibold" className={styles.sellerName} title={item.name}>
