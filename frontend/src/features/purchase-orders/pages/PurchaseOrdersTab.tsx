...
import { getPurchaseOrders, createPurchaseOrder, approvePurchaseOrder, rejectPurchaseOrder, cancelPurchaseOrder, receivePurchaseOrder } from '../api/purchaseOrders.api';
...
export function PurchaseOrdersTab() {
  ...
  const handleApprove = async (id: string) => {
    try { await approvePurchaseOrder(id); showToast('Orden aprobada', 'success'); await loadOrders(); } catch (err: any) { showToast(err.message, 'error'); }
  };

  const handleReject = async (id: string) => {
    const reason = window.prompt('Motivo del rechazo (opcional):');
    try { await rejectPurchaseOrder(id, reason || undefined); showToast('Orden rechazada', 'success'); await loadOrders(); } catch (err: any) { showToast(err.message, 'error'); }
  };

  const handleCancel = async (id: string) => {
    const reason = window.prompt('Motivo de la cancelación (opcional):');
    if (!window.confirm('¿Cancelar esta orden de compra?')) return;
    try { await cancelPurchaseOrder(id, reason || undefined); showToast('Orden cancelada', 'success'); await loadOrders(); } catch (err: any) { showToast(err.message, 'error'); }
  };

  const handleReceive = async (id: string, items: { productId: string; quantity: number }[]) => {
    setReceiving(true);
    try { await receivePurchaseOrder(id, items); showToast('Productos recibidos exitosamente', 'success'); setReceiveTarget(null); await loadOrders(); } catch (err: any) { showToast(err.message, 'error'); } finally { setReceiving(false); }
  };

  const completedOrders = orders.filter(o => o.status === 'completed' || o.status === 'received').length;
  const pendingOrders = orders.filter(o => o.status === 'pending' || o.status === 'approved' || o.status === 'partially_received').length;
  const totalOrders = orders.length;
  const totalInvestmentUsd = orders.filter(o => o.status !== 'cancelled' && o.status !== 'rejected').reduce((sum, o) => sum + Number(o.total), 0);
  ...
  return (
    <>
      ...
      <PurchaseOrdersList
        orders={orders} suppliers={suppliers} loading={loadingOrders} skeletonEnabled={config.skeletonEnabled}
        onApprove={handleApprove} onReject={handleReject} onCancel={handleCancel}
        onReceiveClick={setReceiveTarget} userRole={user?.role}
      />
      <PurchaseOrderForm open={showForm} onClose={() => setShowForm(false)} onSubmit={handleSubmit} loading={loading} />
      <ReceiveModal open={!!receiveTarget} order={receiveTarget} onClose={() => setReceiveTarget(null)} onSubmit={handleReceive} loading={receiving} />
    </>
  );
}
