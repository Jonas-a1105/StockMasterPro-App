...
export function PurchaseOrdersList({
  ...
  return (
    <div className="lista-container">
            ...
            skeletonEnabled ? (
              Array.from({ length: 5 }).map((_, idx) => (
                <tr key={`loader-${idx}`}>
                  ...
                  <td className={styles.flexCenter}><Skeleton height={14} width="60px" /></td>
                  <td><Skeleton height={14} width="100px" /></td>
                  <td></td>
                </tr>
              ))
            ) : (
              <tr><td colSpan={6} className={`${styles.emptyRow} ${styles.p16}`}><LoadingDots text="Cargando órdenes..." /></td></tr>
            )
          ) : (
            orders.map(order => (
              <tr key={order.id}>
                  ...
                  {userRole !== 'cajero' && (
                    <>
                      {order.status === 'pending' && onApprove && (
                        <button onClick={() => onApprove(order.id)} title="Aprobar orden" className={`${styles.actionBtn} ${styles.approveBtn}`}>
                          <Check size={14} /> Aprobar
                        </button>
                      )}
                      {order.status === 'pending' && onReject && (
                        <button onClick={() => onReject(order.id)} title="Rechazar orden" className={`${styles.actionBtn} ${styles.rejectBtn}`}>
                          <X size={14} /> Rechazar
                        </button>
                      )}
                      {['approved', 'partially_received'].includes(order.status) && onReceiveClick && (
                        <button onClick={() => onReceiveClick(order)} title="Recibir productos" className={`${styles.actionBtn} ${styles.receiveBtn}`}>
                          <Package size={14} /> Recibir
                        </button>
                      )}
                      {['pending', 'approved', 'partially_received'].includes(order.status) && onCancel && (
                        <button onClick={() => onCancel(order.id)} title="Cancelar orden" className={`${styles.actionBtn} ${styles.cancelBtn}`}>
      ...
      {!loading && orders.length === 0 && <p className={`${styles.emptyRow} ${styles.p16}`}>No hay órdenes de compra</p>}
    </div>
  );
}
