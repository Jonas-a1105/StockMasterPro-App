...
export function POSPage() {
  ...
  const handleMixedPaymentSubmit = useCallback(
    async (
      payments: {
  ...
  usePosShortcuts({
    ...
    onPaymentMethodChange: (m) => {
      setPaymentMethod(m);
      showToast(`Método de pago: ${m}`, 'info');
    },
  ...
  return (
    <>
      ...
      <div className={styles.posContainer}>
        ...
        <aside className={`${styles.cartPanel} ${cartOpen ? styles.cartPanelOpen : ''}`}>
          ...
          <Cart
            ...
            isOnline={isOnline}
            onPauseOrder={pauseOrder}
            onOpenCash={() => cash.setShowCashModal(true)}
            onOpenExpense={() => cash.setShowExpenseModal(true)}
            carts={carts}
            onResumeOrder={(pc) => {
              cart.setItems(pc.items);
              setCarts((prev) => prev.filter((c) => c.id !== pc.id));
            }}
      ...
      <CashRegisterModal
        show={cash.showCashModal}
        isTodayOpen={cash.isTodayOpen}
        cashOpening={cash.cashOpening}
        cashSalesTotal={cash.cashSalesTotal}
        declaredAmount={cash.declaredAmount}
        onCashOpeningChange={cash.setCashOpening}
        onDeclaredAmountChange={cash.setDeclaredAmount}
        onOpenCash={handleOpenCash}
        onCloseCash={handleCloseCash}
        onClose={() => {
          cash.setShowCashModal(false);
          cash.setDeclaredAmount(0);
          cash.setCashOpening(0);
        }}
      />

      <ExpenseModal show={cash.showExpenseModal} onClose={() => cash.setShowExpenseModal(false)} />
        ...
        <CheckoutModal
          ...
          onPrintTicket={() =>
            printTicket({
              ...
              discount:
                lastSale.subtotal + lastSale.tax - lastSale.total > 0
                  ? lastSale.subtotal + lastSale.tax - lastSale.total
                  : undefined,
            })
          }
        />
