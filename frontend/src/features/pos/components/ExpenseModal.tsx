      ...
      <div className={styles.modalCard} onClick={e => e.stopPropagation()}>
        ...
        <div className={styles.modalBody}>
          <div className={styles.modalField}>
            ...
            <input type="number" className={styles.modalInput} value={amount || ''} onChange={e => setAmount(parseFloat(e.target.value) || 0)} min={0} step={0.01} autoFocus />
          </div>
          <div className={styles.modalField}>
            <label className={styles.modalLabel}>Motivo</label>
            <input type="text" className={styles.modalInput} value={reason} onChange={e => setReason(e.target.value)} placeholder="Describa el gasto..." />
          </div>
        ...
        <div className={styles.modalActions}>
          <button className={styles.modalBtnSecondary} onClick={handleClose}>Cancelar</button>
          <button className={styles.modalBtnPrimary} onClick={handleSave} disabled={amount <= 0 || !reason.trim()}>Registrar Gasto</button>
        </div>
      </div>
    </div>,
    document.body
