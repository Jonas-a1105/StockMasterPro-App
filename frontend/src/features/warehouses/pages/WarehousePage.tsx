...
export function WarehousePage() {
  ...
  if (loading) return config.skeletonEnabled ? <SkeletonTablePage rows={8} cols={5} kpi={3} /> : <LoadingDots text="Cargando almacenes..." />;

  return (
    <div className={styles.container}>
      <TabNav
        tabs={[
          { key: 'almacenes', label: 'Almacenes', icon: <Building2 size={16} /> },
        ]}
        activeTab="almacenes"
        onTabChange={() => {}}
      />
      ...
      <KpiGrid
        items={[
          ...
          { icon: <Package size={18} />, value: activeWarehouses, label: 'Activos', color: '#16a34a' },
          { icon: <MapPin size={18} />, value: totalWarehouses - activeWarehouses, label: 'Inactivos', color: totalWarehouses - activeWarehouses > 0 ? '#dc2626' : '#16a34a' },
        ]}
      />
