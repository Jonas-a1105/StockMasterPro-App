...
export function WarehouseTransfersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      !form.toWarehouseId ||
      !form.items.some((i) => i.productId && i.quantity > 0)
    ) {
    ...
    <div className={styles.container}>
      ...
      <TabNav
        tabs={[
          ...
          {
            key: 'in_transit',
            label: `En tránsito (${counts.in_transit})`,
            icon: <Truck size={16} />,
          },
          {
            key: 'completed',
                ...
                <tr key={t.id}>
                  ...
                  <td className={styles.textCenter}>
                    <span
                      ...
                      style={
                        {
                          '--badge-bg': STATUS_COLORS[t.status] + '20',
      ...
      {modal && (
        <Modal
          ...
          title={modal.type === 'create' ? 'Nueva Transferencia' : 'Detalle de Transferencia'}
          wide
        >
          {modal.type === 'create' && (
            ...
            <div className={styles.viewContent}>
              <div className={styles.detailGrid}>
                <div className={styles.detailField}>
                  ...
                  <span>{modal.data.fromWarehouse?.name}</span>
                </div>
                <div className={styles.detailField}>
                  <span className={styles.detailLabel}>Destino</span>
                  <span>{modal.data.toWarehouse?.name}</span>
                </div>
                <div className={styles.detailField}>
                  ...
                  <span>
                    <span
                      ...
                      style={
                        {
                          '--badge-bg': STATUS_COLORS[modal.data.status] + '20',
                          '--badge-color': STATUS_COLORS[modal.data.status],
                        } as React.CSSProperties
                      }
                    >
                      {STATUS_LABELS[modal.data.status]}
                    </span>
                  </span>
                </div>
                <div className={styles.detailField}>
              ...
              {modal.data.status === 'pending' && (
                <div className={styles.viewActions}>
                  <button
                    ...
                    onClick={() => handleStatusChange(modal.data.id, 'in_transit')}
                  >
                    <Truck size={14} /> Enviar a tránsito
                  </button>
                  <button
                    className={styles.cancelBtn}
                    onClick={() => handleStatusChange(modal.data.id, 'cancelled')}
                  >
                    <X size={14} /> Cancelar
                  </button>
                </div>
              ...
              {modal.data.status === 'in_transit' && (
                <div className={styles.viewActions}>
                  <button
                    ...
                    onClick={() => handleStatusChange(modal.data.id, 'completed')}
                  >
