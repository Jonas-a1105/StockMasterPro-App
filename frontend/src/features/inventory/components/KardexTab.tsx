...
export function KardexTab() {
  ...
  useEffect(() => { getInventoryProducts().then(setProducts).catch(() => {}); }, []);
  ...
  const loadMovements = async (productId: string) => {
    setLoading(true);
    try { setMovements(await getInventoryMovements(productId)); } catch { setMovements([]); } finally { setLoading(false); }
  };
  ...
  const typeLabel = (type: string) => {
    const labels: Record<string, string> = { sale: 'Venta', purchase: 'Compra', adjustment: 'Ajuste', waste: 'Merma', return: 'Devolución', theft: 'Robo' };
    return labels[type] || type;
  };

  return (
    <>
      <div className={styles.header}><h3 className={styles.sectionTitle}>Kardex de Inventario</h3></div>
              ...
              {showDropdown && filteredProducts.length > 0 && (
                <div className={styles.dropdown}>
                  {filteredProducts.map(p => (
                    <div key={p.id} className={styles.dropdownItem} onClick={() => selectProduct(p)}>
                      <span>{p.name}</span><span className={styles.dropdownMeta}>Stock: {p.stock}</span>
                    </div>
                  ))}
                </div>
              )}
      ...
      {selectedProduct && (
        <>
          <div className={styles.kardexInfo}><Package size={18} /><span><strong>{selectedProduct.name}</strong> — Stock actual: {selectedProduct.stock}</span></div>
          {loading ? (
