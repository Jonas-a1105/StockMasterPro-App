  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.cost > 0 && form.price < form.cost) {
      const confirmed = window.confirm(
        `⚠️ El precio de venta ($${form.price.toFixed(2)}) es menor que el costo ($${form.cost.toFixed(2)}).\n\nEsto genera pérdida en cada venta. ¿Desea continuar?`
      );
      if (!confirmed) return;
    }
    setLoading(true);
          ...
          <div className={styles.formGrid}>
            ...
            <div className={styles.field}>
              ...
              {form.cost > 0 && form.price > 0 && form.price < form.cost && (
                <span style={{ color: '#f97316', fontSize: 11, marginTop: 2, display: 'block' }}>
                  ⚠️ Precio por debajo del costo (margen negativo: -${(form.cost - form.price).toFixed(2)})
                </span>
              )}
            </div>
            <div className={styles.field}>
