...
import { SkeletonKPI, SkeletonChart } from '../components/common/Skeleton';
...
function getBestSellers(sales: any[], products: any[]) {
  ...
  sales.forEach(s => {
    (s.items || []).forEach((item: any) => {
      ...
      const name = prod?.name || item.productName || item.productId || 'Producto Desconocido';
...
export function DashboardPage() {
  ...
  const recentActivity = useMemo(() => {
    ...
    sales.slice(0, 4).forEach(s => {
      ...
      items.push({ title: `Venta registrada ${s.invoiceNumber ? `#${s.invoiceNumber}` : ''}`, desc: `Total $${(s.total || 0).toFixed(2)} · ${s.paymentMethod || 'Caja Central'}`, time, color: '#22c55e' });
    });
                  ...
                  {todaySales.slice(0, 3).map((s, i) => (
                    ...
                    >
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-muted, #888)' }}>
                        ${(s.total || 0).toFixed(2)} • {s.paymentMethod || 'Caja Central'}
                      </div>
                    </div>
                  ))}
