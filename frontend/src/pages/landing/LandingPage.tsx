...
import { PLANS, getFeaturesForPlan, type PlanTier } from '../../lib/plan-features';
...
function LazyLottie({ iconKey }: LazyLottieProps) {
  ...
  return (
    <div className={styles.lottieWrap}>
      <div ref={containerRef} className={styles.lottieIcon} onMouseEnter={handleEnter} onMouseLeave={handleLeave} />
    </div>
  );
}

function PricingCard({ tier, isSelected, onSelect }: { tier: PlanTier; isSelected: boolean; onSelect: () => void }) {
      ...
      <div className={styles.cardInner}>
        <div className={styles.cardHeader}>
          <span className={styles.cardLevel}>
            {tier === 'free' ? 'NIVEL_ESENCIAL' : tier === 'pro' ? 'CRECIMIENTO_COMERCIAL' : 'PROTECCIÓN_EMPRESARIAL'}
          </span>
        ...
        <div className={styles.priceRow}>
          ...
          <span className={styles.pricePeriod}>{plan.price === 0 ? ' / indefinido' : ' / mes'}</span>
        </div>
    ...
    <div className={styles.page}>
      <header className={styles.header}>
        <nav className={styles.nav}>
          ...
          <div className={`${styles.navLinks} ${menuOpen ? styles.navOpen : ''}`}>
            <a href="#features" className={styles.navLink} onClick={() => setMenuOpen(false)}>CARACTERÍSTICAS</a>
            <a href="#pricing" className={styles.navLink} onClick={() => setMenuOpen(false)}>PLANES</a>
            <a href="#download" className={styles.navLink} onClick={() => setMenuOpen(false)}>DESCARGAR</a>
            <Link to="/login" className={styles.navLink} onClick={() => setMenuOpen(false)}>INICIAR SESIÓN</Link>
            <Link to="/register" className={styles.navCta} onClick={() => setMenuOpen(false)}>REGISTRARSE</Link>
          </div>
        </nav>
      </header>
      ...
      <section className={styles.hero}>
        <div className={styles.heroContent}>
          ...
          <h1 className={styles.heroTitle}>CONTROLA TU NEGOCIO<br />DESDE CUALQUIER LUGAR</h1>
          <p className={styles.heroSub}>Punto de venta, inventario, finanzas y más — todo en una plataforma moderna.</p>
          <div className={styles.heroActions}>
            <Link to="/register" className={styles.heroCta}>COMENZAR GRATIS</Link>
            <a href="#features" className={styles.heroSecondary}>VER CARACTERÍSTICAS</a>
          </div>
        </div>
      </section>
      ...
      <section id="features" className={styles.sectionAlt}>
        <div className={styles.sectionInner}>
          ...
          <div className={styles.featureGrid}>
            {[
              { icon: '🛒', title: 'PUNTO DE VENTA', desc: 'POS rápido con búsqueda, carritos múltiples y métodos de pago variados.' },
              { icon: '📦', title: 'INVENTARIO', desc: 'Control de stock, órdenes de compra, ajustes, Kardex y múltiples almacenes.' },
              { icon: '📊', title: 'REPORTES', desc: 'Dashboard con KPIs, utilidad neta, best-sellers y productos muertos.' },
              { icon: '👥', title: 'CLIENTES', desc: 'Cartera de clientes con límites de crédito, historial de compras y pagos.' },
              { icon: '💰', title: 'FINANZAS', desc: 'Cuentas por pagar, gastos fijos, notas de crédito y utilidad neta.' },
              { icon: '📱', title: 'OFFLINE / PWA', desc: 'Funciona sin internet. Sincronización automática al recuperar conexión.' },
              { icon: '🔒', title: 'MULTI-TENANT', desc: 'Aislamiento seguro de datos entre empresas con RLS.' },
              { icon: '📅', title: 'AGENDA DIGITAL', desc: 'Calendario de eventos y recordatorios para tu equipo.' },
            ].map(f => (
      ...
      <section id="pricing" className={styles.sectionPricing}>
        <div className={styles.sectionInner}>
          ...
          <div className={styles.pricingGrid}>
            {TIERS.map(tier => (
              <PricingCard key={tier} tier={tier} isSelected={selectedTier === tier} onSelect={() => setSelectedTier(tier)} />
            ))}
          </div>
        </div>
      </section>
