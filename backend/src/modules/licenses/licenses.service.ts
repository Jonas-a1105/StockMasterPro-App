...
import { ActivateLicense } from './core/ActivateLicense';
...
export class LicensesService {
  ...
  async getUsageStats(tenantId: string) {
    ...
    const isIntermedio = tenant.planType === 'enterprise' || tenant.planType === 'intermedio';

    return {
      planType: tenant.planType,
      products: { current: productCount, limit: isFree ? 10 : isIntermedio ? 75 : null },
      users: { current: userCount, limit: isFree ? 1 : null },
      warehouses: { current: warehouseCount, limit: (isFree || isIntermedio) ? 1 : null },
      customers: { current: customerCount, limit: isFree ? 2 : isIntermedio ? 15 : null },
      suppliers: { current: supplierCount, limit: isFree ? 2 : isIntermedio ? 15 : null },
      accountsPayable: { current: accountsPayableCount, limit: isFree ? 0 : isIntermedio ? 15 : null },
      expenses: { current: expensesCount, limit: isFree ? 0 : isIntermedio ? 10 : null },
      salesThisMonth,
      storageUsage,
    };
  }
  ...
  async upgradePlan(tenantId: string, newPlan: string) {
    ...
    if (newIndex <= currentIndex) {
      throw new HttpException('El nuevo plan debe ser superior al actual', HttpStatus.BAD_REQUEST);
    }
    return this.licenseRepo.updatePlan(tenantId, newPlan);
  }

  async generate(dto: { targetTenantId?: string; days: number; tier?: string }) {
    const payload = {
    ...
    const code = this.jwtService.sign(payload, {
      secret: process.env.LICENSE_JWT_SECRET || 'stockmaster-license-secret',
    });
  ...
  async reactivate(tenantId: string) {
    ...
    if (tenant.licenseExpiresAt && new Date() > tenant.licenseExpiresAt) {
      throw new Error('No se puede reactivar: la licencia ya expiró. Genere un nuevo código de activación.');
    }
    await this.licenseRepo.markAsActive(tenantId);
    return { message: 'Suscripción reactivada correctamente', expiresAt: tenant.licenseExpiresAt };
  }
}
