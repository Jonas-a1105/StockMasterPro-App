...
import { AccountsPayableRepository, CreatePayableData, PayablePaymentData } from '../../application/ports/AccountsPayableRepository.interface';
...
export class PostgresAccountsPayableRepo implements AccountsPayableRepository {
  ...
  async updatePendingAmount(id: string, tenantId: string, amount: number): Promise<void> {
    const existing = await this.prisma.accountsPayable.findFirst({ where: { id, tenantId } });
