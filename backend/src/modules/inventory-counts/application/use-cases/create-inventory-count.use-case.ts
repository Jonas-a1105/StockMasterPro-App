...
import { InventoryCount, InventoryCountItem, InventoryCountNotFoundException } from '@modules/inventory-counts';
...
export class CreateInventoryCountUseCase {
  ...
  async execute(input: CreateCountInput): Promise<InventoryCount> {
    if (input.warehouseId) {
      const warehouse = await this.warehouseService.findById(input.warehouseId, input.tenantId);
      if (!warehouse) throw new Error('Almacén no encontrado');
    }

    const productWarehouseMap = new Map<string, { productId: string; warehouseId: string }>();

    if (input.productIds?.length) {
      const products = await this.productRepo.findByIds(input.productIds, input.tenantId);
      for (const product of products) {
        const pws = await this.productRepo.getProductWarehouses(product.id, input.tenantId);
        for (const pw of pws) {
          productWarehouseMap.set(pw.id, { productId: product.id, warehouseId: pw.warehouseId });
        }
      }
    }
    ...
    if (input.productWarehouseIds?.length) {
      for (const pwId of input.productWarehouseIds) {
        const pw = await this.productRepo.getProductWarehouseById(pwId, input.tenantId);
        if (pw) productWarehouseMap.set(pw.id, { productId: pw.productId, warehouseId: pw.warehouseId });
      }
    }
