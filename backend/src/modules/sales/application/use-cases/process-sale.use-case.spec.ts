...
import { ProductNotFoundException, InsufficientStockException, InvalidSaleOperationException } from '../../domain/sales.errors';
...
describe('ProcessSaleUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new ProcessSaleUseCase(saleRepo, productRepo, receivableRepo, cashRepo);
  });
  ...
  it('should throw if product not found', async () => {
    ...
    await expect(useCase.execute(baseInput)).rejects.toThrow(ProductNotFoundException);
  });
  ...
  it('should process sale with multiple items', async () => {
    const products = [
      mockProduct,
      { ...mockProduct, id: 'prod-2', name: 'Product 2', price: 50, cost: 30, stock: 20 },
    ];
